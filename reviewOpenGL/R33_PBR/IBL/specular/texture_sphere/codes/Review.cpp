...
int main()
{
    glm::vec2 a0 = glm::vec2(2.0, 3.0);
    glm::vec2 a1 = glm::vec2(4.0, 6.0);
    cout << glm::to_string(a0 * a1) << endl;
...
//    pbrShader.setVec3("albedo", glm::vec3(1.0, 1.0, 1.0));
    ...
    unsigned int sphereAlbedo = loadTexture("Sphere/rustediron2_basecolor.png");
    unsigned int sphereMetallic = loadTexture("Sphere/rustediron2_metallic.png");
    unsigned int sphereNormal = loadTexture("Sphere/rustediron2_normal.png");
    unsigned int sphereRoughness = loadTexture("Sphere/rustediron2_roughness.png");
    ...
    pbrShader.setInt("albedoTexture", 3);
    pbrShader.setInt("metallicTexture", 4);
    pbrShader.setInt("normalTexture", 5);
    pbrShader.setInt("roughnessTexture", 6);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_CUBE_MAP, irradianceMap);
    ...
    glBindTexture(GL_TEXTURE_2D, sphereAlbedo);
    glActiveTexture(GL_TEXTURE4);
    glBindTexture(GL_TEXTURE_2D, sphereMetallic);
    glActiveTexture(GL_TEXTURE5);
    glBindTexture(GL_TEXTURE_2D, sphereNormal);
    glActiveTexture(GL_TEXTURE6);
    glBindTexture(GL_TEXTURE_2D, sphereRoughness);
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        // render multiple sphere with different roughness and metallic
//        for(int i = 0; i < 6; i++){
//            pbrShader.setFloat("roughness", glm::clamp((float)i / 6.0, 0.05, 1.0));
//            for (int j = 0; j < 6; j++) {
//                model = glm::mat4(1.0);
//                model = glm::translate(model, glm::vec3(i - 3, j - 3, -2.0) * glm::vec3(2.5));
//                pbrShader.setMat4("model", model);
//                pbrShader.setFloat("metallic", (float)j / 6.0);
//                renderSphere();
//            }
//        }
