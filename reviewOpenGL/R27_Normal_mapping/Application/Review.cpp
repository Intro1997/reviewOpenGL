...
int main()
{
    ...
    // Load floor texture
    // ------------------
    unsigned int wallNormalMapTexture = loadTexture("img/brickwall_normal.jpg");
    unsigned int wallMapTexture = loadTexture("img/brickwall.jpg");
...
//    wallShader.setInt("wallTexture", 0);
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        glBindTexture(GL_TEXTURE_2D, wallMapTexture);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, wallNormalMapTexture);
        renderQuad(shader);
