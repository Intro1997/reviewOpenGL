...
int main()
{
    ...
    // -------------------------
    ReviewShader cubeShader("shader/model.vs", "shader/model.fs");
    ReviewShader glassShader("shader/model.vs", "shader/glass.fs");
    ...
    glassShader.use();
    glassShader.setInt("glassTexture", 0);
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        map<float, glm::vec3> order;
        
        for(int i = 0; i < vegetation.size(); i++){
            float distance = glm::length(camera.Position - vegetation[i]);
            order[distance] = vegetation[i];
        }
        ...
        for(map<float, glm::vec3>::reverse_iterator it = order.rbegin(); it != order.rend(); ++it){
            glassShader.use();
            model = glm::mat4(1.0f);
